...
pub trait InvoiceApi: Send + Sync {
    ...
    ) -> Result<models::CreatedInvoice, Error<CreateInvoiceError>>;
    ...
    ) -> Result<models::Invoice, Error<GetInvoiceError>>;
    ...
    ) -> Result<models::InvoiceList, Error<GetInvoicesError>>;
}
...
/// struct for passing parameters to the method [`InvoiceApi::create_invoice`]
#[derive(Clone, Debug)]
#[cfg_attr(feature = "bon", derive(::bon::Builder))]
pub struct CreateInvoiceParams {
    pub create_invoice: Option<models::CreateInvoice>,
}

/// struct for passing parameters to the method [`InvoiceApi::get_invoice`]
#[derive(Clone, Debug)]
#[cfg_attr(feature = "bon", derive(::bon::Builder))]
...
/// struct for passing parameters to the method [`InvoiceApi::get_invoices`]
#[derive(Clone, Debug)]
#[cfg_attr(feature = "bon", derive(::bon::Builder))]
pub struct GetInvoicesParams {
    ...
    /// Only retrieve the invoice with this number.
    pub invoice_number: Option<String>,
    /// Only retrieve invoices from this relation.
    pub relation_id: Option<i32>,
    /// Only retrieve invoices on this date.
    pub date: Option<String>,
}

#[async_trait]
impl InvoiceApi for InvoiceApiClient {
    /// Create a new invoice.
    async fn create_invoice(
        &self,
        params: CreateInvoiceParams,
    ) -> Result<models::CreatedInvoice, Error<CreateInvoiceError>> {
        let CreateInvoiceParams { create_invoice } = params;

        let local_var_configuration = &self.configuration;

        let local_var_client = &local_var_configuration.client;

        let local_var_uri_str = format!("{}/v1/invoice", local_var_configuration.base_path);
        ...
        local_var_req_builder = local_var_req_builder.json(&create_invoice);
            ...
            match local_var_content_type {
                ...
                ContentType::Text => {
                    return Err(Error::from(serde_json::Error::custom(
                        "Received `text/plain` content type response that cannot be converted to `models::CreatedInvoice`",
                    )));
                }
                ContentType::Unsupported(local_var_unknown_type) => {
                    return Err(Error::from(serde_json::Error::custom(format!(
                        "Received `{local_var_unknown_type}` content type response that cannot be converted to `models::CreatedInvoice`"
                    ))));
                }
            }
        } else {
    ...
    async fn get_invoice(
        ...
        let GetInvoiceParams { id } = params;
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Text => {
                    return Err(Error::from(serde_json::Error::custom(
                        "Received `text/plain` content type response that cannot be converted to `models::Invoice`",
                    )));
                }
                ContentType::Unsupported(local_var_unknown_type) => {
                    return Err(Error::from(serde_json::Error::custom(format!(
                        "Received `{local_var_unknown_type}` content type response that cannot be converted to `models::Invoice`"
                    ))));
                }
            }
        } else {
            let local_var_entity: Option<GetInvoiceError> =
                serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent {
                status: local_var_status,
                content: local_var_content,
    ...
    async fn get_invoices(
        ...
        if let Some(ref param_value) = relation_id {
            local_var_req_builder =
                local_var_req_builder.query(&[("relationId", &param_value.to_string())]);
        }
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Text => {
                    return Err(Error::from(serde_json::Error::custom(
                        "Received `text/plain` content type response that cannot be converted to `models::InvoiceList`",
                    )));
                }
                ContentType::Unsupported(local_var_unknown_type) => {
                    return Err(Error::from(serde_json::Error::custom(format!(
                        "Received `{local_var_unknown_type}` content type response that cannot be converted to `models::InvoiceList`"
                    ))));
                }
            }
        } else {
...
/// struct for typed errors of method [`InvoiceApi::create_invoice`]
...
/// struct for typed errors of method [`InvoiceApi::get_invoice`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
/// struct for typed errors of method [`InvoiceApi::get_invoices`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
